...
```powershell
powershell -ExecutionPolicy Bypass -File E:\Base3D\glTF-SDK\Benchmarks\LoadExport\RunMatchedBenchmarks.ps1 `
...
The comparison/reporting orchestrator is kept on `Release/2.0.0`, where the
durable cross-branch evidence belongs. It invokes this branch's executable on
every alternating cycle, rejects toolchain/configuration mismatches, and
writes raw CSV, environment JSON, canonical output hashes, summary JSON, and a
Markdown comparison under
